...
<script>
    ...
    function add() {
        hk.open("${ctx}/system/city/form/add","城市信息新增");
    };

    function edit() {
        var id = hk.getSelectId();
        if(id.length > 1){
            alert("只能选择一条城市信息进行编辑!!!")
        }else {
            hk.open("${ctx}/system/city/form/edit?id="+id,"城市信息编辑");
        }
    };
    ...
    function deleteAll() {
        ...
        if(ids.length >= 1){
            var b = confirm("确认删除城市信息吗？");
            if (b){
                hk.get("${ctx}/system/city/deleteAll?ids="+ids,function (data) {
                    hk.info("删除成功");
                    citytable.draw( false );/*重新绘制表格*/
                });
            }
        }
    };
    ...
    function view() {
        ...
        if(id.length > 1){
            ...
            hk.open("${ctx}/system/city/form/view?id="+id,"用户浏览");
        }
    };
</script>
